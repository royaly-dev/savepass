...
async function opensettings() {
   ...
   <svg class="svg-cross" width="30px" height="30px" viewBox="0 -0.5 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div id="settings-container">
         <label id="loadjson">Import Password</label>
         <label id="exportjson">Export Password</label>
      </div>
   </div>
   `
   ...
   setTimeout(() => {
      const crossSvg = document.querySelector(".svg-cross")
      crossSvg.addEventListener("click", () => document.getElementById("settings").remove())
      const importpass = document.getElementById("loadjson")
      importpass.addEventListener("click", async () => {
         const folderPaths = await window.db.selectFiles();
         loadNewJson(folderPaths[0])
      })

      const exportpass = document.getElementById("exportjson")
      exportpass.addEventListener("click", async () => {
         const folderPaths = await window.db.selectDirs();
         
         exportNewJson(folderPaths[0] + "\\SavePassExport.json")
      })
   }, 200);
}