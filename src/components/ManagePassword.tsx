...
export default function ManagePassword(props: {data: Data, updateData?: PasswordData, type: string, requestPassword: boolean, refresh(): void, openChange(): void} ) {
    ...
    const [password, setPassword] = useState<PasswordData>({id: (crypto as any).randomUUID(), password: "", url: "", mail: "", deleted: false})
    ...
    const savepassord = () => {
        try {
            ...
            if (new URL(password.url).hostname) {
                ...
                props.openChange()
                setPassword({id: (crypto as any).randomUUID(), password: "", url: "", mail: "", deleted: false})
            }
    ...
    return (
        <Dialog modal={false} open={props.requestPassword && isReady} onOpenChange={() => {props.openChange(); setPassword({id: (crypto as any).randomUUID(), password: "", url: "", mail: "", deleted: false})}} >
