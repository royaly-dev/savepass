...
export default function ManageTotp(props: { data: Data, request: boolean, refresh(): void, openChange(): void }) {
    ...
    useEffect(() => {
        setTotp({ key: "", id: (crypto as any).randomUUID(), name: "", provider: "", deleted: false })
        if (props.request) {
    ...
    return (
        <Dialog modal={false} open={props.request && isReady} onOpenChange={() => { props.openChange(); setTotp({ key: "", id: (crypto as any).randomUUID(), name: "", provider: "", deleted: false }) }} >
