...
const mainInstance = instance.find({ type: "http" }, (Service: Service) => {
  ...
  const isLocal = Object.values(networkInterfaces()).flat().filter((item) => item?.address === Service.referer?.address)
  if (isLocal.length === 0 && Service.name.includes("savepass") && Services.filter(item => item.host === Service.host).length === 0 && Service?.addresses.length > 0 || (Service?.txt?.readytosync && isLocal.length === 0)) {
